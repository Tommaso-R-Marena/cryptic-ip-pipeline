...
The `notebooks/` directory contains five Colab-ready notebooks. Click an
**Open in Colab** badge to launch the notebook directly in Google Colab
(targets the `main` branch so links remain stable after merge):
...
The badge links target the `main` branch so they keep working after this
PR is merged. The external scientific binaries (fpocket / FreeSASA /
APBS / PyMOL) are not available on a vanilla Colab runtime; the
notebooks document the fallback behaviour and a `condacolab + mamba`
install path. See [`docs/colab.md`](docs/colab.md) for the full guide.
